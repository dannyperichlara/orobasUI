...
let randomNumbers = new Array(10000).fill(0)
...
AI.PVS = function (board, alpha, beta, depth, ply, allowNullMove, illegalMovesSoFar, lookForMateTurn) {
    ...
    for (let i = 0, len = moves.length; i < len; i++) {
        ...
        if (board.makeMove(move)) {
            ...
            // Late moves reductions (+251 ELO)
            if (!mateE) {
                R += AI.LMR_TABLE[depth][legal]
                
                // History reductions (+81 ELO)
...
AI.search = function (board, options) {
    ...
    // AI.createTables(board, true, true, true, true) //XXXXXXX
    ...
    return new Promise((resolve, reject) => {
        ...
        let candidateMoveIndex = candidateMoves.length - 1

        if (candidateMoveIndex < 0) candidateMoveIndex = 0

        AI.bestmove = candidateMoves[candidateMoveIndex]
        ...
        if (!AI.bestmove) {
            ...
            orobas.loadFen(AI.originalFEN)

            AI.createTables(board, true, true, true, true)

            AI.nullWindowFactor = 20

            AI.PVS(board, -INFINITY, INFINITY, 6, 1, false, 0, false)
            // (board, alpha, beta, depth, ply, allowNullMove, illegalMovesSoFar, lookForMateTurn) {
            let ttEntry = AI.ttGet(board.turn, board.hashkey)

            console.log('New Entry:', ttEntry)
            ...
            if (++totalNoMoves > 10) postMessage({nobestmove: true})
        }
